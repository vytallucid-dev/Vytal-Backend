...
/**
 * ═══════════════════════════════════════════════════════════════════════════════════════════════════
 * ★★ THE GLOBAL COUNTER SCOPE — THE MODEL **AND** WHICH KEY IS SPENDING IT.
 *
 * ⚠ IT WAS THE MODEL ID ALONE, AND THAT MADE THE COUNTER KEY-BLIND. `ai_usage_counters` records OUR
 *   SPEND, which equals THE KEY'S REMAINING QUOTA only while the key is constant. The moment a key is
 *   rotated the two diverge: the old key's total keeps being charged against a new key that has a full
 *   day of its own. Measured live — a rotation left the row reading 461 of 480 against a fresh key,
 *   and the only way through was resetting the row BY HAND. A ceiling that has to be corrected
 *   manually after an ordinary operation is not a ceiling.
 *
 * ★ THE FINGERPRINT MAKES ROTATION SELF-CORRECTING. A different key is a different scope, so it opens
 *   its own window at zero and the old key's row is left intact as history. Nothing to remember, and
 *   nothing to reset.
 *
 * ⚠⚠ IT IS A ONE-WAY HASH, TRUNCATED, AND THE KEY ITSELF NEVER LEAVES THIS FUNCTION. The scope string
 *    is written to a table, read by gates and printed in reports; a key fragment in any of those is a
 *    credential leak with a long tail. Eight hex characters of SHA-256 distinguish keys without being
 *    reversible or being enough of the key to matter.
 *
 * ★ AND `nokey` IS A REAL VALUE, NOT A BUG. With no key configured there is nothing to meter against —
 *   the mock provider is unmetered by `spendFor` — and a stable scope keeps that case from opening a
 *   new window on every boot.
 * ═══════════════════════════════════════════════════════════════════════════════════════════════════
 */
export function globalScopeOf(model: string): string {
  const key = process.env.GEMINI_API_KEY ?? process.env.GOOGLE_API_KEY ?? "";
  if (!key) return `${model}#nokey`;
  return `${model}#${createHash("sha256").update(key).digest("hex").slice(0, 8)}`;
}
...
export async function checkAndConsumeAiCall(model: string, actor: Actor): Promise<QuotaDecision> {
  // ★ ONE HOME (N-3). This and `peekAiCallQuota` built the scope independently, which is how the two
  //   would have drifted the first time either changed.
  const globalScope = globalScopeOf(model);
