// ═══════════════════════════════════════════════════════════════════════════════════════════════════
// USER-DIRECTED MEMORY — the service seam behind rememberThis / listMemories / forgetMemory.
//
// Service-shaped for the same reason the Stage 3 write tools are (write-shared.ts): the RULES live here
// and the tools only translate conversation into arguments. A tool that re-declared "no personal
// circumstance" would be a second home for the most important rule in this file.
//
// ★ TWO PROVENANCES, ONE LIST. `chat_reader_profile` holds what the distiller INFERRED (register, depth
// nudge, glossary gaps) and now also what the reader STATED. `listMemories` renders BOTH as one list,
// every entry carrying its `source`, because:
//   · "tell me what you remember" is only honest if the reader can tell an inference from their own words.
//     Someone shown "you prefer Hinglish" that they never said must be able to see it was worked out.
//   · deletion has to work uniformly. "Forget that I prefer Hinglish" is an INFERRED field; "forget the
//     one about explanations" is a STATED item. If they resolved through different lists, the feature
//     would silently only half-work.
//
// ★ AND A THIRD STORE, WHICH IS NEITHER: `statedName`. A form of address is not a preference about
// explanations and it is not an inference — it is an instruction about how to talk to someone. It lives on
// its own column, it is written by the same `rememberThis` the reader already uses, and it renders into
// the SAME unified list as `source: "stated"` so "what do you remember about me?" stays one honest answer.
// See §"THE SELF-NAMING CARVE-OUT" for why it bypasses the decline gate entirely.
// ═══════════════════════════════════════════════════════════════════════════════════════════════════
import { prisma } from "../db/prisma.js";
import { ServiceError } from "../lib/service-error.js";
import { VYTAL_VOCAB_KEYS } from "./profile.js";
...
export async function statedMemoriesFor(userId: string): Promise<string[]> {
  const p = await prisma.chatReaderProfile.findUnique({ where: { userId }, select: { statedMemories: true } });
  return readStated(p?.statedMemories).map((m) => m.text);
}

/**
 * The requested form of address, for injection into the orientation header. null ⇒ the reader never asked
 * for one and the account name stands.
 *
 * ★ NOT BEHIND `AI_PROFILE_INJECT`, AND THAT IS THE POINT. The flag gates what was INFERRED about the
 * reader — register, depth, glossary gaps — because those are a model's guesses and shipping them early
 * would ship untuned guesses. A name the reader typed out and confirmed is not a guess; it is an
 * instruction, and holding it behind an inference flag would mean obeying "call me Ronaldo" only after an
 * unrelated switch is thrown. It rides beside `user_ledger.display_name`, which is already outside the
 * flag for exactly the same reason.
 *
 * Trimmed and clamped to the column's own 40-char CHECK. Fail-soft: a read error simply omits the clause.
 */
export async function statedNameFor(userId: string): Promise<string | null> {
  try {
    const p = await prisma.chatReaderProfile.findUnique({ where: { userId }, select: { statedName: true } });
    const n = p?.statedName?.trim();
