...
export interface QuarterPoint {
  ...
  // Depreciation → PBT → Tax → Net Profit. All ₹ Cr; zeroToNull-guarded.
  otherIncome: number | null; // ₹ Cr
  expenses: number | null; // ₹ Cr (total expenses)
  depreciation: number | null; // ₹ Cr
  interest: number | null; // ₹ Cr (finance costs, stored as `interest` on QuarterlyResult)
...
export interface AnnualSnapshot {
  ...
  profitBeforeTax: number | null; // ₹ Cr
  tax: number | null; // ₹ Cr
  ebitda: number | null; // ₹ Cr (derived, stored on Fundamental)
