...
//               statement as another scalar subquery rather than as a third round trip. The filing
//               channel's rows (below) went into that SAME statement as one more CTE, for the same
//               reason: verify-personal-section.ts counts these round trips and asserts 0 / 1 / 2.
//
// ── ★★ THE FINDING-CHANGE FACT NOW READS THE LIVE CHANNEL, AND THE TRANSITION IS THE HARD PART ────
// It used to diff score_red_flags between this quarter's snapshot and the prior one. NOTHING WRITES
// score_red_flags any more — every red-flag rule (R1…R6) is a filing rule and moved to stock_findings
// at migration 20260809120000 — so that diff could only ever return "unchanged", forever, on rows
// frozen at cutover. It reads stock_findings now.
//
// ⚠⚠ AND A STANDING STATE IS NOT A TRANSITION. The pass stamps `newly_standing` whenever the prior row
// did not fire, INCLUDING when there is no prior row at all — so on a channel that has been backfilled
// one period per grain, every single fired row in the table reads newly_standing (481 of 481, measured).
// Believing that literally would tell a reader that every check Vytal has ever run on their holding
// started flagging this quarter. The alert channel already refuses exactly this case, in so many words
// (filing/read.ts, readNewlyStandingFilingKeys): a row counts as newly appeared only when the
// (stock, rule) pair has a STRICTLY-OLDER PERIOD ON RECORD. This module applies the identical test.
//
// ★ THE STOPPED SIDE NEEDS NO EXTRA GUARD, AND IT IS WORTH SAYING WHY. `resolved` is stamped only when
// the prior row FIRED (filing/pass.ts, resolveStanding), so it cannot be a first observation by
// construction — the prior-period test is already inside the state. It is applied anyway, so both
// halves of the sentence are gated on the same measurable fact rather than one of them on a proof.
//
...
// ★ THE FILING CHANNEL'S REGISTRY, READ AS A PREDICATE — never a second key list. See the
//   findingChanges block below for what this module does with it.
...
export async function buildPersonalSection(
  ...
  const detail = (await prisma.$queryRaw<
    {
      ...
      /** This stock's whole red-flag series from the FILING channel, newest period first. Reduced to
       *  a started/stopped pair in TypeScript — see `filingRedFlagTransitions`. */
      filing_rows: unknown;
      book_moves: unknown;
  ...
  >`
    ...
    -- ★ THE FILING CHANNEL, IN THE SAME ROUND TRIP. Every red-flag row this company has, every period,
    -- with NO reduction here: which row is current and whether a strictly-older one exists is decided
    -- in TypeScript, against the same rule the alert evaluator applies. Doing it in SQL would put a
    -- second copy of that rule in a language where nothing would notice it drifting from the first.
    -- ⚠ period_end IS THE ORDERING KEY, NOT period_key. The filing period is grain-prefixed
    -- ("S:FY27Q1", "A:FY26") and does NOT sort across grains — schema.prisma says so on the column.
    -- ⚠ RED FLAGS ONLY, which is what this sentence has always counted: it replaces a score_red_flags
    -- diff. Patterns are a different claim and are not silently folded in here.
    filing AS (
      SELECT rule_key,
             period_end,
             evaluation_state::text AS evaluation_state,
             standing_state::text   AS standing_state
      FROM stock_findings
      WHERE stock_id = ${stockId} AND kind::text = 'red_flag'
    )
    SELECT
      ...
      -- WAS: two NOT-IN counts over score_red_flags between this snapshot and the prior one. That
      -- table is dead — see the header — so the diff decayed to "unchanged" on frozen rows. The rows
      -- come back RAW and the transition is computed in code.
      (SELECT json_agg(
                json_build_object('ruleKey', rule_key, 'state', evaluation_state, 'standing', standing_state)
                ORDER BY period_end DESC)
         FROM filing) AS filing_rows,
  ...
  // ── 4a · CHECKS THAT STARTED OR STOPPED FLAGGING ON A STOCK THEY HOLD ──────────────────────────
  // ⚠ COUNTS, NOT NAMES. Naming them would print the same list twice on one card wherever they are
  // already listed. What this section adds is the one thing no finding list can say — that it
  // happened on THEIR stock.
  // ⚠ AND ONLY ON A REAL TRANSITION. See the header: `newly_standing` alone is a first observation on
  // every pair the backfill has touched once, and a first observation is not a change.
  const { started: startedN, stopped: stoppedN } = filingRedFlagTransitions(detail?.filing_rows);
  if ((holds || watches) && startedN + stoppedN > 0) {
    const parts: string[] = [];
    if (startedN > 0) parts.push(`${startedN} of Vytal's filing checks started flagging`);
    if (stoppedN > 0) parts.push(`${stoppedN} ${startedN > 0 ? "stopped" : "of Vytal's filing checks stopped flagging"}`);
    facts.push({
      key: "personal.findingChanges",
      kind: "change",
      // WAS: "…on this quarter's score. You hold this company, and the health section above names
      // them." Both halves became false in the same move. These are FILING findings — they belong to
      // the company's own filing period, not to this quarter's score — and the health block on THIS
      // page is the score channel with the filing keys suppressed (filing/channel.ts), so it does not
      // name them. A pointer a reader cannot follow is worse than no pointer, so it is gone rather
      // than redirected: the stock's own health section is where these are listed.
      display:
        `${parts.join(" and ")} on this company's latest filings. ` +
        `You ${holds ? "hold" : "are watching"} this company.`,
    });
  }
...
/**
 * ★ THE FILING CHANNEL'S TWO TRANSITIONS, AND THE PRIOR-PERIOD TEST THAT MAKES THEM TRANSITIONS.
 *
 * Rows arrive `period_end` DESC, so the FIRST row for a rule is its current state and any further row
 * for the same rule is a strictly-older period. That ordering is the whole test, and it is the same
 * one filing/read.ts's `readNewlyStandingFilingKeys` performs for the alert evaluator — reproduced
 * here rather than called because this module has already paid for these rows inside a statement it
 * was making anyway, and a second query would break the round-trip budget the header states.
 *
 *   started — current row is fired + newly_standing, AND a second row exists for that rule.
 *   stopped — current row is resolved, AND a second row exists for that rule.
 *
 * ⚠ THE CANDIDATE IS DELETED WHEN IT IS PROMOTED, so a rule with three periods on record is counted
 * ONCE, on its second row, and never again.
 *
 * ⚠ EVERY KEY IS CHECKED AGAINST THE FILING REGISTRY, which is the same predicate — read the other
 * way round — that every dual-channel surface uses to suppress these keys on the score side. A key
 * this table carries that the filing pass does not own would be some other channel's transition
 * counted as this one's.
 */
export function filingRedFlagTransitions(raw: unknown): { started: number; stopped: number } {
  if (!Array.isArray(raw)) return { started: 0, stopped: 0 };
  const seen = new Set<string>();
  const candidate = new Map<string, "started" | "stopped">();
  let started = 0;
  let stopped = 0;
  for (const row of raw) {
    if (!row || typeof row !== "object") continue;
    const { ruleKey, state, standing } = row as Record<string, unknown>;
    if (typeof ruleKey !== "string" || !isFilingChannelKey(ruleKey)) continue;
    if (!seen.has(ruleKey)) {
      seen.add(ruleKey);
      if (state === "fired" && standing === "newly_standing") candidate.set(ruleKey, "started");
      else if (state === "not_fired" && standing === "resolved") candidate.set(ruleKey, "stopped");
      continue;
    }
    // A second row for this rule ⇒ a prior period exists ⇒ the candidate is a real transition.
    const kind = candidate.get(ruleKey);
    if (kind === undefined) continue;
    candidate.delete(ruleKey);
    if (kind === "started") started++;
    else stopped++;
  }
