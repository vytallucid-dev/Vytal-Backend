    ...
    for (const m of [...pg.members].sort((a, b) => a.symbol.localeCompare(b.symbol))) {
      members[m.symbol] = {
        ...
        findings: s(
          (m.findings ?? [])
            .map((f) => ({
              // `metricRefs` was in this fingerprint until 2026-08-10 and left with the column. A
              // baseline written by an older tree therefore cannot be diffed against a new one —
              // which was already true of any tree whose rules have changed, since both files are
              // produced on demand by whichever tree is being compared.
              ...
              polarity: f.polarity ?? null, evidence: f.evidence,
            }))
