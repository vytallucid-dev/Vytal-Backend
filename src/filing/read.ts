    ...
    const fired: FilingFindingView[] = current
      ...
      .map((r) => {
        ...
        return {
          ...
          kind: r.kind === "red_flag" ? ("red flag" as const) : ("pattern" as const),
          severity: r.severity,
      ...
      .sort((a, b) =>
        (a.kind === b.kind ? 0 : a.kind === "red flag" ? -1 : 1) ||
...
/**
 * ★ WHICH FILING FINDINGS NEWLY APPEARED — the alert evaluator's input.
 *
 ...
 * strictly-older period on record. Today that is true for zero pairs — the backfill has run one period
 * per grain — which is exactly the honest answer: nothing has newly appeared yet.
 */
export async function readNewlyStandingFilingKeys(stockIds: readonly string[]): Promise<Map<string, Set<string>>> {
  ...
  const rows = await prisma.stockFinding.findMany({
    where: { stockId: { in: [...stockIds] } },
    orderBy: [{ stockId: "asc" }, { periodEnd: "desc" }],
    select: { stockId: true, ruleKey: true, evaluationState: true, standingState: true },
  });
  ...
  for (const r of rows) {
    ...
    if (!seen.has(pair)) {
      seen.add(pair);
      if (r.evaluationState === "fired" && r.standingState === "newly_standing") newly.set(pair, r.stockId);
      continue;
    }
    // A second row for this pair ⇒ a prior period exists ⇒ the candidate is a real transition.
    const stockId = newly.get(pair);
    if (stockId === undefined) continue;
    newly.delete(pair);
    const set = out.get(stockId) ?? new Set<string>();
    set.add(pair.slice(pair.indexOf("|") + 1));
    out.set(stockId, set);
  }
...
export function filingDefinitions(sections: Iterable<FilingFindingsSection>): FilingFindingDefinition[] {
  ...
  for (const s of sections) for (const f of s.fired) keys.add(f.ruleKey);
  return [...keys]
    .map((key) => ({
