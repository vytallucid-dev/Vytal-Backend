...
async function buildHealthSnapshotViewInScope(
  ...
  //
  // ★ CHANNEL SUPPRESSION (filing/channel.ts) — SECOND predicate, different reason. A filing-rule row
  //   frozen on this head is not obsolete: it is TRUE, and `filingFindings` above is already showing
  //   it, from the filing's own period. Serving it here too renders the same finding as two cards with
  //   two dates. The row stays in score_red_flags as history; only what this reader is shown changes.
  //
  // ⚠ APPLIED HERE AND NOT AT `firedHeadlines` (≈:924), DELIBERATELY. That set drives the three-lens
  //   anti-double-count: a lens read is suppressed when a headline finding already covers its ground.
  //   The finding still covers that ground — it is still on this page, in the other channel — so
  //   filtering it there would resurrect lens cards the reader was correctly not being shown, which
  //   is a scored surface MOVING rather than de-duplicating.
  const redFlags: RedFlagView[] = dropFilingFlags(dropRetiredFlags(snap.redFlags))
    .map((rf) => ({
      flagKey: rf.flagKey,
