...
async function main() {
  ...
  // ── §1b · AGAINST WHAT SHIPPED, AT THE PERIOD IT SHIPPED FOR ─────────────────────────────────
  // A persisted RedFlag is a statement about the quarter it was written for, NOT about today.
  // INDUSINDBK's row is from 2025-09-30 and its pledge has since fallen below the line; comparing it
  // against a fresh evaluation would compare two different quarters and call the difference a defect.
  // Each row is therefore replayed against the shareholding prefix the pillar actually saw.
  const persisted = await prisma.redFlag.findMany({
    where: { flagKey: "ownership_R1_pledge" },
    orderBy: { createdAt: "desc" },
    select: { symbol: true, severity: true, triggeringValues: true, asOfDate: true },
  });
  const latestBySymbol = new Map<string, (typeof persisted)[number]>();
  for (const p of persisted) if (!latestBySymbol.has(p.symbol)) latestBySymbol.set(p.symbol, p);
  console.log(`\n  persisted score_red_flags R1 rows: ${persisted.length} across ${latestBySymbol.size} stock(s)`);
  for (const [symbol, row] of latestBySymbol) {
    const subj = universe.find((u) => u.symbol === symbol);
    if (!subj) continue;
    const all = shByStock.get(subj.stockId) ?? [];
    const idx = all.map((q) => q.asOnDate.getTime() <= row.asOfDate.getTime()).lastIndexOf(true);
    const rows = idx >= 0 ? all.slice(0, idx + 1) : [];
    const r = ruleR1(shCtx(subj.stockId, symbol, rows));
    const f = r !== null && !isNotEvaluable(r) ? r : null;
    const at = rows.length ? `${rows[rows.length - 1].fiscalYear}${rows[rows.length - 1].quarter}` : "—";

    // ⚠ COMPARE ON THE KEYS THE PERSISTED ROW ACTUALLY CARRIES, and account for the ones it does not.
    // `verdict` was added to R1's triggeringValues on 2026-07-29 (§1.6 — before that R1 shipped only
    // engineer-facing `reasons`, and the card rendered "A red flag is standing on this stock"). Rows
    // written before that date have no `verdict` key. Demanding one would be asserting that an old
    // row was written by today's code, which is a fact about the calendar, not about these two
    // implementations agreeing.
    const pv = (row.triggeringValues ?? {}) as Record<string, unknown>;
    const rv = (f?.evidence ?? {}) as Record<string, unknown>;
    const sharedSame = !!f && Object.keys(pv).every((k) => canon(pv[k]) === canon(rv[k]));
    const extraKeys = f ? Object.keys(rv).filter((k) => !(k in pv)) : [];
    const onlyVerdictMissing = extraKeys.length === 0 || (extraKeys.length === 1 && extraKeys[0] === "verdict");
    const same = !!f && row.severity === f.severity && sharedSame && onlyVerdictMissing;
    const note = extraKeys.includes("verdict") ? "  [pre-2026-07-29 row: no `verdict` key existed yet]" : "";
    ok(same, `${symbol} @${at} (row as-of ${row.asOfDate.toISOString().slice(0, 10)}): every value the persisted RedFlag carries is reproduced by the rule${note}`);
    if (!same) {
      console.log(`       persisted = ${canon(row.triggeringValues)}`);
      console.log(`       rule      = ${f ? canon(f.evidence) : "DID NOT FIRE"}`);
    }
  }
