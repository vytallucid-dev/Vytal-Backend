// ═══════════════════════════════════════════════════════════════════════════════════════════════════
// EVENT DESCRIPTION PARSER — the fix for a number that was reachable in PROSE but absent from STRUCTURE.
//
// ★★★ THE DEFECT, MEASURED LIVE, IN ONE LINE. ★★★
// A corporate_events row carried `dividendAmount = 11`, `dividendType = "interim"`, and this description:
//
//     "Interim Dividend Rs 11 Per Share/ Special Dividend Rs 46 Per Share"
//
// The ₹46 special existed ONLY in that free-text tail. Asked for TCS's dividend history, the model read
// the tail, added the two, and wrote "totaling 57 rupees per share". **₹57 appears in no column of this
// database.** It violates CLOSED_WORLD_HEADER ("every number you state must appear verbatim below") and
// CONVERSATIONAL_PRECISION ("never compute a new number of your own") — and it did so while looking
// completely correct, which is what makes it expensive. FY25's row hides a ₹66 special the same way, FY24
// an ₹18, FY23 a ₹67.
//
// ── ⚠ THE NON-OBVIOUS PART: STRUCTURING THE COMPONENTS IS NOT ENOUGH. ──────────────────────────────
// The instinct is "parse the ₹46 into a field and we're done". It is not: with 11 and 46 both in
// structure and no 57 anywhere, the model can STILL add them. Every number it might reasonably state has
// to be in front of it, so this parser emits the TOTAL as well. Arithmetic belongs in code, where it is
// reviewable and deterministic, never in a language model that was told not to do arithmetic.
//
// ── THE THREE OUTCOMES, AND WHY THE THIRD IS FAIL-CLOSED ──────────────────────────────────────────
//   · "clean"      — the description carries no currency amount at all (board meetings, AGM notices).
//                    Passed through verbatim. 2,691 of 7,741 rows.
//   · "structured" — EVERY amount in the text is attributed to a labelled dividend component. The caller
//                    renders the components + the total and DROPS the raw tail (it is now redundant).
//   · "suppress"   — an amount is present that no component claims, or the row is a revision. The caller
//                    must DROP the tail and say so. It must NEVER pass the tail through: a description
//                    carrying an unattributable number is the exact configuration that produced ₹57.
//
// ⚠ FAIL-CLOSED MEANS LOSING REAL INFORMATION SOMETIMES, AND THAT IS THE PRICE. A face-value split
// ("From Rs 10/- To Rs 2/-") is suppressed rather than parsed: `splitRatio` already carries the split in
// structure, and those rupee figures are a FACE VALUE, not a payment — inventing a field for them would
// solve a problem nobody has. A "(Purpose Revised)" row is suppressed even though it parses, because a
// revision may REPLACE rather than ADD, and a total we cannot justify is the very thing this file exists
// to prevent.
//
// PURE. No I/O, no DB, no clock. Every branch is decided from the string and the structured amount.
// ═══════════════════════════════════════════════════════════════════════════════════════════════════
