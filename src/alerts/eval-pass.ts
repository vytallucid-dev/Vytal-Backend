 ...
 *   diff) · score_patterns (finding keys on those snapshots; score_red_flags was here and was
 *   dropped 2026-08-11 — red flags come from the filing channel) ·
...
export async function assembleReadings(stockIds: string[]): Promise<Map<string, StockReading>> {
  ...
  // ★ THE RED-FLAG READ IS GONE (2026-08-11). It was already returning nothing that survived:
  //   `dropFilingFlags` stripped every row it could produce, because all six red-flag rules are
  //   filing rules. The filing channel supplies those keys to this pass separately (see the
  //   newly-standing read below), so removing the query changes no alert's evaluation.
  const patterns =
    snapIds.length === 0
      ? []
