    ...
    for (const ref of SCORED_PGS) {
      ...
      for (const m of pg.members) {
        ...
        const nowFind = JSON.parse(s(
          // ⚠ `metricRefs` left this projection on 2026-08-10 with the column. It must stay in step
          //   with filing-pass-step1-parity.ts's fingerprint — the two shapes are diffed against each
          //   other, so a field dropped from one and kept in the other reads as "every finding
          //   CHANGED".
          (m.findings ?? []).map((f) => ({
