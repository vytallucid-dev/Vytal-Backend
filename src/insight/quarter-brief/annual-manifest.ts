...
export const ANNUAL_MANIFEST = {
  ...
  banking: [
    ...
    { key: "basicEps", source: "banking_fundamentals.basic_eps", scale: "perShare",
      ...
      note: "⚠⚠ `book_value_per_share` IS DELIBERATELY ABSENT FROM EVERY MANIFEST IN THIS FILE, and this is where the reason is on file. KOTAKBANK FY25 stores a book value per share of ₹117,239.89 — which is its ENTIRE NET WORTH IN ₹ CRORE, because `face_value_share` on that row is 994.11, the same number as `paid_up_equity_capital`. The parser wrote the paid-up capital into the face-value slot, the derived share count came out as one, and the ratio became the numerator. No single-metric bound catches it: MRF's real book value is ₹49,468 a share, so any bound loose enough to keep MRF admits Kotak at twice the magnitude. It is only implausible BESIDE its own face value — the cet1/tier1 shape exactly. Net worth in rupees says the same thing without a share count in the denominator, so the per-share form is dropped rather than guarded. ⚠ The parse itself belongs in the XBRL parser against the filed source (PB-3, xbrl/parser-backlog.ts) — dropping the metric contains it, it does not fix it." },
  ],
