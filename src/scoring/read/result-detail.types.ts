...
// ★ THE SECOND FINDING CHANNEL — see the field note on ResultHealthBlock.filingFindings.
...
export interface ResultHealthBlock {
  ...
  /**
   * ★ THE FILING CHANNEL — and on THIS surface it is the closer of the two channels to the subject.
   *
   * The viewer is looking at a FILING. `findings` above is dated by the latest SNAPSHOT's quarter,
   * which is why this block carries `latestPeriodKey` and the frontend has to caveat the list as
   * "latest health findings" whenever the viewed quarter is older. A filing finding is dated by the
   * filing's OWN period (`period` / `periodEnd` / `grainLabel` on each row), so it can be stated
   * against the result being viewed rather than beside it.
   *
   * Present on the unscored path too — `scored: false` means no composite, not no filings. The block
   * used to say "no health composite or findings to show for this result" on those stocks, and the
   * second half of that sentence stopped being true when the 22 rules moved channels.
   */
