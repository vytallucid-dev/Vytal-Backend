// ═══════════════════════════════════════════════════════════════════════
// PORTFOLIO HEALTH VIEW — the read-assembly service for the authenticated user's PHS read.
//
// buildPortfolioHealthView(userId) is the reusable seam behind GET /api/v1/me/portfolio: it
// reads the latest persisted snapshot + the live disclosure + the read-time findings and
// REGROUPS them into the two-read contract. The controller is a thin auth → call → envelope
// shell; a future AI grounding layer calls THIS to read exactly what the portfolio page shows.
//
// PURE READ over the persisted snapshot (A.12). It NEVER computes or persists a score — it
// only serves the latest persisted row + live disclosure. The snapshot is the single source of
// truth (engine → persist.computeAndPersistPhs).
//
...
// the only two things that move portfolio health. So by the time this read runs, the
...
import { loadHeldInstrumentFacts, loadHeldFundAnalytics } from "./read-time-catalog.js";
import { describeUnpriced } from "../disclosures.js";
...
// the engine module.
import { constructionBandOf, type ConstructionBand } from "./constants.js";
import type { Archetype, Exposures, CDeduction, ConstructionData, EntityLedgerEntry } from "./entity.js";
...
export interface ConstructionRead {
  ...
  /**
   * ★ THE ENTITY LEDGER, FIRST-CLASS — every aggregated name-risk entity (issuer) with its display
   * name, book weight, resolved sector, and constituent instruments. Verbatim off `constructionData`;
   * the same array `composeStory` already reads below. Nothing is recomputed and nothing is derived.
   *
   * ⚠ WHY IT IS SURFACED RATHER THAN SCAVENGED. Until now the only structured carrier of the held
   * COMPANY names in this payload was PA3's `bind.entities` — and PA3 is a DUPLICATE-ISSUER detector:
   * it fires only when `entityCount < nameRiskInstruments` (patterns.ts), i.e. when some issuer is held
   * through two or more instruments. An ordinary book of one-instrument-per-company never fires it, so
   * a consumer reading the names off PA3 sees the full list on some books and NOTHING on others, with
   * no signal that anything is missing. That is the shape a grounding layer must never be handed: a
   * fact that is silently absent reads as "the user holds no companies" rather than "ask elsewhere".
   * PA1 fires always but binds only the entity COUNT, not the list. So the ledger gets its own field.
   *
   * null ⇔ no `constructionData` (pre-2.0 / no-holding rows) — we do not know the ledger, exactly as
   * `rules`/`exposures` degrade. `[]` is DIFFERENT and also honest: a fund-only book has a genuinely
   * empty name-risk sleeve. Never fabricated either way.
   */
  entities: EntityLedgerEntry[] | null;
...
export function reshapeSnapshot(s: SnapshotReadInput, counts: ReshapeCounts, readTime: PfFinding[]): PortfolioReads {
  ...
  const constructionRead: ConstructionRead = {
    ...
    // The entity ledger, verbatim — the SAME array fed to composeStory below (zero new data access,
    // zero recompute). `Array.isArray` guards the pre-2.0 rows where the key is absent entirely.
    entities: Array.isArray(cData?.entities) ? cData.entities : null,
...
/**
 * THE PHS READ SEAM. Assembles the `data` payload GET /api/v1/me/portfolio serves — the latest
 * persisted snapshot regrouped into the two-read contract + the live disclosure + read-time findings.
 * Reads only (zero writes): a GET NEVER computes or persists. Owner is passed in (the controller derives
 * it from req.authUser.userId — IDOR-proof). Throws on failure; the controller maps that to a 500.
 */
export async function buildPortfolioHealthView(userId: string) {
  ...
  if (!snap) {
    ...
    return { snapshot: null, hasHoldings: disclosure.positionCount > 0, disclosure: disclosureOut, referenceFindings: disclosureFindings };
  }
...
/** The `data` object shape GET /api/v1/me/portfolio wraps in `{ success, data }`. Inferred from the
 *  service so it can never drift from what is actually returned. */
export type PortfolioHealthView = Awaited<ReturnType<typeof buildPortfolioHealthView>>;
