...
async function main(): Promise<void> {
  ...
  if (!probe) {
    ...
    ok(
      ...
      !!leanRow && Array.isArray(leanRow.patterns) && Array.isArray(leanRow.redFlags) && !!leanRow.bandMappingVersion,
      leanRow ? `${leanRow.patterns.length} patterns · ${leanRow.redFlags.length} flags` : "missing",
    );
  }
